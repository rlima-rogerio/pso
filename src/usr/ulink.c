...
//
//    /* Acceleration data (keep as raw ADC - accelerometer specific) */
//    dp->accel[0] = (int16_t)adc0_buffer[0];  /* x-axis vibration */
//    dp->accel[1] = (int16_t)adc1_buffer[0];  /* y-axis vibration */
//    dp->accel[2] = (int16_t)adc1_buffer[1];  /* z-axis vibration */
//
...
//
//    /* UPDATED: Current in amperes (0-6600 A) ✓ */
//    dp->i_motor = current_adc_to_a(adc1_buffer[2]);
//
//    /* UPDATED: Voltage in decivolts (0-334 dV = 0-33.4V) ✓ */
//    dp->v_motor = voltage_adc_to_dv(adc0_buffer[2]);
//
//    /* Thrust (keep as raw ADC - load cell specific) */
//    dp->thrust = (int16_t)adc0_buffer[1];    /* Force, [N] - needs calibration */
//
//    /* Throttle (no change) */
//    dp->throttle = g_pwm_value;              /* Throttle, [0 ~ 100] */
//
//    return returnval;
//}
