...
*
* DESCRIPTION:
*       ULINK communication protocol implementation for PSO system
*
* AUTHOR:      Rogerio Lima
* VERSION:     1.2 - Dec 2025 - Fixed header dependencies
*
******************************************************************************/
...
#include "pso_uart.h"   /* Must come before pso_data.h */
#include "pso_data.h"   /* For PACKET_LENGTH definition */
#include "ulink.h"
...
* Description    : Write message to UART Tx buffer
******************************************************************************/
void uart_write(void)
{
    ulink_pso_data_t dp_local;
    ...
    dp_local.accel[0] = (int16_t)adc0_buffer[0];  /* x-axis vibration */
    dp_local.accel[1] = (int16_t)adc1_buffer[0];  /* y-axis vibration */
    dp_local.accel[2] = (int16_t)adc1_buffer[1];  /* z-axis vibration */
    dp_local.rpm = 1750U;                         /* Revolutions per minute */
    dp_local.i_motor = (int16_t)adc1_buffer[2];   /* Current of the motor, [A] */
    dp_local.v_motor = (int16_t)adc0_buffer[2];   /* Voltage applied to the motor, [V] */
    dp_local.thrust = (int16_t)adc0_buffer[1];    /* Force generated by the motor + propeller, [N] */

    memset(uart_tx_buffer, 0, sizeof(uart_tx_buffer));

    create_message(ulink_system.sysid, ulink_system.compid, uart_tx_buffer, &dp_local);
}

/******************************************************************************
* Function Name  : uart_read
* Description    : Read from UART Rx buffer and translate message to command
...
/******************************************************************************
* Function Name  : create_checksum
* Description    : Create CRC checksum for message
******************************************************************************/
...
/******************************************************************************
* Function Name  : accumulate_checksum
* Description    : Accumulate one byte of data into the CRC
******************************************************************************/
...
* Description    : Parse ULINK message and extract command
...
uint8_t parse_message(uint16_t *msg, ulink_pso_command_t *command)
{
    ...
    checksum = create_checksum(msg, ULINK_MSG_ID_PSO_COMMAND_LEN + ULINK_CORE_HEADER_LEN);
...
* Description    : Copy PSO data to UART transmission buffer
******************************************************************************/
void copy_data(uint16_t* uart_tx_buf, ulink_pso_data_t* dp)
{
    ...
    memset(uart_tx_buf, 0, PACKET_LENGTH * sizeof(uint16_t));
...
* Description    : Packages sensor data into PSO data packet structure
*******************************************************************************/
uint8_t packet_data(ulink_pso_data_t* dp)
{
    uint8_t returnval = 1U;  /* Initialize to success */
